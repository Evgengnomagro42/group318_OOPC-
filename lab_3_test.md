...
## а) 
### X код с ошибками
int return 5(){  // надо без пробела return5 или с _ return_5
 return 5        // надо ; в конце строки
 }
int return8(){
 return;         //не указано, что надо вернуть, а просто прервать выполнение программы не имеет смысла
}
int main( {      //нужна )
